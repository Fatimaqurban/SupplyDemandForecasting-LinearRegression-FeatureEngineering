# -*- coding: utf-8 -*-
"""SupplyDemandForecasting-FeatureEngineering.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1uOyL2N_DsLw1adBFmSX0uko2FRmi0zyL
"""

from google.colab import drive
drive.mount('/content/drive')

# Define the headers for your order data files
order_data_headers = [
    "Order_ID",
    "Driver_ID",
    "Passenger_ID",
...
# Define the header for your cluster map file
...
# Load the cluster map with the specified headers
...
# Define the folder path where the order data files are stored
order_data_folder = '/content/drive/My Drive/AI_assignment_3/training_data/order_data/'

# List all files in the folder matching the pattern "order_data_YYYY-MM-DD"
order_data_files = [f for f in os.listdir(order_data_folder) if f.startswith("order_data_")]

# Load all order data files with the specified headers and concatenate them
order_data = pd.concat(
    [pd.read_table(os.path.join(order_data_folder, f), sep='\t', names=order_data_headers, header=None)
     for f in order_data_files],
    ignore_index=True
)


# Merge order data with cluster map to get region IDs
...
def get_time_slot(dt):
    # Define base time (start of the day)
...
regression_data = gap_data[['Region_ID', 'day', 'slot', 'gap']]
...
# Predict the gap for the test data
...
# Predict the gaps for the test data
...
print("kNN Mean Absolute Error:", mae2)
print("kNN Root Mean Squared Error:", rmse2)
print("kNN R-squared:", r_squared2)
...
# Predict the gaps for the test data using Decision Tree
